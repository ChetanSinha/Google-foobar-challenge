...
def solution(lst):
    ...
    def filter_values(a, b):
        quotient = int((a+b)/gcd(a,b))
        return (quotient & quotient-1) != 0
    
    def paired(key, taken,seen):
        for value in dic[key]:
            if seen[value] == False:
                seen[value] = True
                if taken[value] == -1 or paired(taken[value], taken, seen):
                    taken[value] = key
                    return True
        return False
...
#     GRAPH
    dic = {}
    for row_ind in range(len(lst)):
        temp = []
        for col_ind in range(row_ind, len(lst)):
            if filter_values(lst[row_ind], lst[col_ind]):
                if row_ind in dic:
                    dic[row_ind].append(col_ind)
                else:
                    dic[row_ind] = [col_ind]
                
                if col_ind in dic:
                    dic[col_ind].append(row_ind)
                else:
                    dic[col_ind] = [row_ind]
    ...
    for key, values in dic.items():
        seen = [False]*len(lst)
        if paired(key, taken, seen):
            answer += 1
            
    return len(lst) - 2 * (answer//2) 
